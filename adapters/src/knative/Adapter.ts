...
export class KnativeAdapter
  implements AbstractAdapter<KnativeRoute, KnativeEnvironment>
{
  private readonly logger = new Signale();

  public async handleRoute(
    environment: KnativeEnvironment,
    route: KnativeRoute
  ): Promise<void> {
    const tmpDir = resolvePath(
      process.cwd(),
      "tmp",
      encodeURI(route.entrypoint)
    );
  ...
  public static route(
    options: KnativeRouteConfiguration,
  ...
  public static routes(
    list: Array<KnativeRouteConfiguration>,
    environmentId?: string
  ): Array<KnativeRoute> {
