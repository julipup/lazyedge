...
export class ExecutionContext {
  ...
  public readonly spaces: Map<
    string,
    Map<string, AbstractEnvironment<AbstractRoute>>
  > = new Map();
  public readonly routes: Map<
    String,
    AbstractRoute & { spaceIds: string[]; willBeDeployed: boolean }
  > = new Map();

  constructor(public readonly spaceId = "default") {}
  ...
  public async checkRoute(route: AbstractRoute) {
    ...
    if (this.routes.has(route.entrypoint)) {
      const entriesIndecies = this.configuration!.routes.map(
        (mappedRoute, index) =>
          mappedRoute.entrypoint == route.entrypoint ? index : null
  ...
  public async checkEnvironment(
    environment: AbstractEnvironment<AbstractRoute>
  ) {
  ...
  ): AbstractEnvironment<AbstractRoute> | null;
  public getEnvironment(
    environmentId: string,
    spaceId = "default",
    doThrowError = true
  ...
  public findEnvironments(
    environmentId: string
  ): Array<AbstractEnvironment<AbstractRoute>> {
