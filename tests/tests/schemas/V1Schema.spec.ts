...
test.group('Configuration -> Config Parsing', (group) => {
    ...
    test('try to parse schema from file exported using CommonJS', ({ assert }) => {
        assert.doesNotThrows(() => {
            ConfigFileImporter
                .from(resolvePath(__dirname, './commonjs/config.js'))
                .asConfig();
        });
    });
    ...
    test('try to import INVALID file exported using CommonJS', ({ assert }) => {
        assert.throws(() => {
            ConfigFileImporter
                .from(resolvePath(__dirname, './commonjs/config.invalid.js'))
                .asConfig();
        });
    });

    test('try to parse schema from file exported using ESModules', ({ assert }) => {
        assert.doesNotThrows(() => {
            ConfigFileImporter
                .from(resolvePath(__dirname, './es/config.js'))
                .asConfig(); 
        });
    });
    ...
    test('try to import INVALID file exported using ESModules', ({ assert }) => {
        assert.throws(() => {
            ConfigFileImporter
                .from(resolvePath(__dirname, './es/config.invalid.js'))
                .asConfig();
        });
    });
    ...
    test('try to parse .ts config', ({ assert }) => {
        assert.doesNotThrows(() => {
            ConfigFileImporter
                .from(resolvePath(__dirname, './misc/ts-config.ts'))
                .asConfig();
        });
    });
});
...
test.group('Configuration -> Annotations', (group) => {
    ...
    test('route\'s string annotation', ({ assert }) => {
        const config = new SchemaParser(
            schemaWithAnnotation("string", "world"),
        );
    ...
    test('route\'s array of strings annotation', ({ assert }) => {
        ...
        assert.deepEqual(config.raw().routes[0].annotations!.array, ["hello", "world"]);
    });

    test('routes\'s boolean annotation', ({ assert }) => {
        const config = new SchemaParser(
            schemaWithAnnotation("isBoolean", true),
        );
    ...
    test('route\'s number annotation', ({ assert }) => {
        const config = new SchemaParser(
            schemaWithAnnotation("number", 10),
        );
    ...
    test('route\'s static computed function annotation', ({ assert }) => {
        ...
        assert.equal(config.computed().routes[0].annotations!.function, 'hewwo dogs!');        
    });
});